...
namespace ms
{
	...
	public class Point<T>
	{
		...
		{
			return new Point<T>(a - v.a, b - v.b).length();
		}
		...
		//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
		//ORIGINAL LINE: constexpr bool operator != (const Point<T>& v) const
		public static bool operator !=(Point ImpliedObject, Point<T> v)
		{
		...
		public static Point<T> operator -(Point ImpliedObject)
		{
			return
			{
		...
		public static Point<T> operator +(Point ImpliedObject, T v)
		{
			return
			{
		...
		public static Point<T> operator +(Point ImpliedObject, Point<T> v)
		{
			return
			{
