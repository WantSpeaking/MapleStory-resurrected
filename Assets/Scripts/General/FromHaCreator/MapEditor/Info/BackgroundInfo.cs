...
namespace HaCreator.MapEditor.Info
{
    public class BackgroundInfo : MapleDrawableInfo
    {
        ...
        private BackgroundInfo(WzImageProperty imageProperty, Bitmap image, MapleLib.WzLib.Point origin, string bS, BackgroundInfoType _type, string no, WzObject parentObject,
        ...
        private static BackgroundInfo Load(GraphicsDevice graphicsDevice, WzImageProperty parentObject, string bS, BackgroundInfoType type, string no)
        {
            ...
            else if (type == BackgroundInfoType.Spine)
            {
                ...
                if (spineCanvas != null)
                {
                    ...
                    return new BackgroundInfo(parentObject, bitmap, WzInfoTools.PointFToPoint (origin__), bS, type, no, parentObject, wzSpineAnimationItem);
                }
                else
                {
                    PointF origin_ = new PointF();
                    //return new BackgroundInfo(parentObject, Properties.Resources.placeholder, WzInfoTools.PointFToPoint (origin_), bS, type, no, parentObject, null);
                    return new BackgroundInfo(parentObject, null, WzInfoTools.PointFToPoint (origin_), bS, type, no, parentObject, null);
                }
            }
            else
            ...
            return new BackgroundInfo(frame0, frame0.GetLinkedWzCanvasBitmap(), WzInfoTools.PointFToPoint (origin), bS, type, no, parentObject, null);
        }
