...
namespace HaCreator.MapEditor
{
    public class Board
    {
        ...
        //private MapleLib.WzLib.Bitmap miniMap;
        private MapleLib.WzLib.Point miniMapPos;
        ...
        /*public static MapleLib.WzLib.Bitmap ResizeImage(MapleLib.WzLib.Bitmap FullsizeImage, float coeff)
        {
            return (MapleLib.WzLib.Bitmap)FullsizeImage.GetThumbnailImage((int)Math.Round(FullsizeImage.Width / coeff), (int)Math.Round(FullsizeImage.Height / coeff), null, IntPtr.Zero);
        }

        public static MapleLib.WzLib.Bitmap CropImage(MapleLib.WzLib.Bitmap img, MapleLib.WzLib.Rectangle selection)
        {
            MapleLib.WzLib.Bitmap result = new MapleLib.WzLib.Bitmap(selection.Width, selection.Height);
            using (MapleLib.WzLib.Graphics g = MapleLib.WzLib.Graphics.FromImage(result))
            {
                g.DrawImage(img, new MapleLib.WzLib.Rectangle(0, 0, selection.Width, selection.Height), selection, MapleLib.WzLib.GraphicsUnit.Pixel);
            }
            return result;
        }*/
        ...
        public bool RegenerateMinimap()
        {
	        ...
	        /*try
	        {
	            lock (parent)
	            {
	                ...
	                {
	                    MapleLib.WzLib.Bitmap bmp = new MapleLib.WzLib.Bitmap(mapSize.X, mapSize.Y);
	                    MapleLib.WzLib.Graphics processor = MapleLib.WzLib.Graphics.FromImage(bmp);
	                    foreach (BoardItem item in BoardItems.TileObjs)
	                    {
	                        processor.DrawImage(item.Image, new MapleLib.WzLib.Point(item.X + centerPoint.X - item.Origin.X, item.Y + centerPoint.Y - item.Origin.Y));
	                    }
	                    bmp = CropImage(bmp, new MapleLib.WzLib.Rectangle(MinimapRectangle.X + centerPoint.X, MinimapRectangle.Y + centerPoint.Y, MinimapRectangle.Width, MinimapRectangle.Height));
	                    MiniMap = ResizeImage(bmp, (float)_mag);
	                    MinimapPosition = new MapleLib.WzLib.Point(MinimapRectangle.X, MinimapRectangle.Y);
	                }
	            }
	            return true;
	        }
        ...
        public MapleLib.WzLib.Point MinimapPosition
        {
