...
namespace ms
{
    ...
    {
        private static readonly MapleDataProvider mapleData = MapleDataProviderFactory.getDataProvider("/Map.wz");

        public Background(MapleData src, bool front)////back1stChildMap/Map1/100000000.img/back/0
        {
            ...
            animated = src["ani"];//animatedMap/Map1/100000000.img/back/0/ani
                                  //801040004.img/ back/0/bS 值为空

            var bsd = src["bS"];
            var bsd1 = src["bS"].ToString();

            string bs = $"{src["bS"]}";

            var ani_data = mapleData.getData($"Back/{src["bS"]}.img")?[animated ? "ani" : "back"]?[src["no"]];

            animation = new Animation(ani_data, GameUtil.layerNameDefault, front ? GameUtil.Instance.sortingLayerName_Foreground : GameUtil.Instance.sortingLayerName_Background); //animationMap.wz/Back/{Map/Map1/100000000.img/back/0/bS}.img/(ani|back)/{Map/Map1/100000000.img/back/0/no}   Map.wz/Back/grassySoil.img/ani/0
        ...
        public void drawTileOnTilemap(double viewx, double viewy, float alpha, UnityEngine.Tilemaps.Tilemap tilemap, UnityEngine.Tilemaps.TileBase tile, int z)
        {
            //animation.draw(new DrawArgument(new Point_short((short)x, (short)y), flipped, opacity / 255), alpha);
            ...
            for (tx = 0; tx < tw; tx += cx)
            {
                for (ty = 0; ty < th; ty += cy)
                {
                    tilemap.SetTile(new Vector3Int((int)ix + tx, (int)iy + ty, z), tile);
                    /*drawCount++;
                    Animation a;
                    if (drawCount > anim_cache.Count)
                    {
        ...
        public string get_fullpath() => fullPath;
    ...
    {
        GameObject gobj_Fore;
        GameObject gobj_Back;
        string o_assetPath = "";
        string t_assetPath = "";

        public MapBackgrounds(MapleData src, int mapid)//back2NodePathMap/Map1/100000000.img/back
        {
            if (!GameUtil.Instance.Use_wz_or_ab_draw_ObjTile)
            {
                if (src is MapleData img100000001_back)
                {
                    for (int i = 0; i < img100000001_back.Children.Count; i++)
                    {
                        if (img100000001_back[i.ToString()] is MapleData img100000001_back_0)
                        {
                            bool front = img100000001_back_0["front"]; //frontMap/Map1/100000000.img/back/0/front
                            Background background = new Background(img100000001_back_0, front);
            ...
            else
            {
                ...
                gobj_Back = UnityEngine.Object.Instantiate<GameObject>(Resources.Load<GameObject>($"Prefabs/Back/Map{prefix}_Back.{mapid}"));
                gobj_Fore = UnityEngine.Object.Instantiate<GameObject>(Resources.Load<GameObject>($"Prefabs/Fore/Map{prefix}_Fore.{mapid}"));
            }
        ...
        public MapBackgrounds(WzObject src, int mapid)//back2NodePathMap/Map1/100000000.img/back
        {
            if (!GameUtil.Instance.Use_wz_or_ab_draw_ObjTile)
            {
                if (src is WzSubProperty img100000001_back)
                {
                    foreach (var wzProperty in img100000001_back.WzProperties)//directoryap/Map1/100000000.img/back/0
                    {

                    }

                    for (int i = 0; i < img100000001_back.WzProperties.Count; i++)
                    {
                        if (img100000001_back[i.ToString()] is WzSubProperty img100000001_back_0)
                        {
                            bool front = img100000001_back_0["front"]; //frontMap/Map1/100000000.img/back/0/front
                            Background background = new Background(img100000001_back_0, front);
        ...
        public void drawbackgrounds(double viewx, double viewy, float alpha)
        {
            ...
            {
                if (gobj_Back == null) return;
                gobj_Back.transform.position = new Vector3((float)viewx, (float)-viewy, 0);
            }
        }
        public void drawforegrounds(double viewx, double viewy, float alpha)
        {
            ...
            {
                if (gobj_Fore == null) return;
                gobj_Fore.transform.position = new Vector3((float)viewx, (float)-viewy, 0);
            }
        }
        public void update()
        {
        ...
        public void Dispose()
        {
            ...
            {
                UnityEngine.Object.Destroy(gobj_Back);
                UnityEngine.Object.Destroy(gobj_Fore);
            }
            AppDebug.Log("Dispose MapBackgrounds");
        ...
        public IList<Background> get_backgrounds()
        { return backgrounds; }
        public IList<Background> get_foregrounds()
        { return foregrounds; }
    }
