...
public class TestAsyncSocketClient2
{
	...
	private void Start ()
	{
		/*bs.WriteShort (7575);
		bs.Position = 0;
		AppDebug.Log ($"bs:{bs.ToArray ().ToDebugLog ()}\t value:{bs.ReadShort ()}");*/

		/*writeShort (7575);
		bs.Position = 0;
		AppDebug.Log ($"bs:{bs.ToArray ().ToDebugLog ()}\t value:{readShort()}");*/

		bs = new MMO_MemoryStream (sbuffer.ToByteArray ());
		//bs.Position = 0;
		//AppDebug.Log ($"bs:{bs.ToArray ().ToDebugLog ()}\t value:{readShort ()}");

		//AppDebug.Log ($"{-105 & 255} \t {-105 & 0xff}\t {sbuffer[0] & 0xFF}\t {(sbuffer[1] & 0xFF) << 8}\t {(sbuffer[0] & 0xFF) + ((sbuffer[1] & 0xFF) << 8)}");
		//AppDebug.Log ($"{sbuffer[0] & 0xFF}\t {(sbuffer[1] & 0xFF) << 8}\t {(sbuffer[0] & 0xFF) + ((sbuffer[1] & 0xFF) << 8)}");
		AppDebug.Log ($"{unchecked((short)64971)}");
