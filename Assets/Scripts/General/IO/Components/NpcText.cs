...
/*
	size_t NpcText::parseL(string& str, size_t pos)
	{
		...
		if (next != string::npos)
		{
			try
			{
				//int32_t selection = stoi(str.substr(pos + 2, next - pos + 2));
	...
	size_t NpcText::parsez(string& str, size_t pos)
	{
		...
		if (next != string::npos)
		{
			try
			{
				const ItemData& idata = ItemData::get(stoi(str.substr(pos + 2, next)));
				if (idata.is_valid())
				{
	...
	size_t NpcText::parseh(string& str, size_t pos)
	{
		...
		if (next != string::npos)
		{
			try
			{
				int32_t type = stoi(str.substr(pos + 2, next - pos + 2));
				string insert;
				switch (type)
				{
				case 0:
	...
	size_t NpcText::parsev(string& str, size_t pos)
	{
		...
		if (next != string::npos)
		{
			try
			{
				if (const ItemData& idata = ItemData::get(stoi(str.substr(pos + 2, next))))
				{
