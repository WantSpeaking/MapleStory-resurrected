			...
			if (src != null && src.IsTexture())
			{
				...
				if (GameUtil.Instance.Use_ab_or_assetDatabase)
				{
					...
					else
					{
						...
						var spriteName = $"{imgName}{fullPath.Substring(dotimgIndex + 4)}";//BasicEff.img\\AranGetSkill\\3
						spriteName = spriteName.Replace("\\", "-").Replace($"{imgName}-", "");//BasicEff.img-AranGetSkill-3
				...
				else
				{
					...
					sprite = TextureAndSpriteUtil.LoadSpriteFromAssetDatabase(abPath.Replace("wzpng\\", ""), assetPath);
#endif
				}
				if (sprite == null)
				{
					AppDebug.LogError($"sprite is null, fullpath:{fullPath}");
				}
				/*if (texture2D == null)
                {
