	...
	/*public class SpriteBatch : SingletonMono<SpriteBatch>
	{
		...
		private void FinalDeSpawn (UnityPoolItem unityPoolItem)
		{
			...
			/*	var transform = unityPoolItem.PooledRef.GetComponent<Transform> ().Component;
				renderer.gameObject.name = Constants.get ().defaultSpriteDrawerName;#1#
			renderer.sprite = null;
			/*renderer.flipY = false;
			renderer.sortingLayerName = Constants.get ().sortingLayer_Default.ToString ();
			renderer.sortingOrder = 0;
			transform.position = Vector3.zero;
			transform.localScale = Vector3.zero;#1#
		}
		...
		private void Draw ()
		{
			/*clearBuffer.Clear ();
			clearBuffer.ClearRenderTarget (true, true, clearColor);
			Graphics.ExecuteCommandBuffer (clearBuffer);#1#
			...
			for (; batchQueue.Count > 0; batchQueue.TryDequeue (out var de))
			{
				if (batchQueue.TryPeek (out var batchItem))
				{
					textureRange = new Rectangle_short ((short)(batchItem.realposX), (short)(batchItem.realposY), (short)batchItem.width, (short)batchItem.height);
					...
					{
						{
							...
							SpriteBatcher.Instance.DrawSpriteQuad (spriteQuad);#1#
						...
						{
							/#1#/var scale = batchItem.data.Length / (batchItem.height * batchItem.width);
													var height = batchItem.height;
													var width = batchItem.width;
													var pixelData = batchItem.data;
			...
			background.Apply ();
			DrawTextureToTarget (background);#1#
