...
namespace ms
{
	...
	public class DENY_PARTY_REQUEST_Packet : OutPacket
	{
		public DENY_PARTY_REQUEST_Packet (string message) : base ((short)OutPacket.Opcode.DENY_PARTY_REQUEST)
		{
			write_byte (0);
	...
	public class MoveMobPacket : MovementPacket
	{
		...
		public MoveMobPacket (int objectid, short moveid, sbyte pNibbles, sbyte rawActivity, sbyte skillId, sbyte skillLv, short pOption, Point_short startpos, Movement movement) : base (OutPacket.Opcode.MOVE_MONSTER)
		{
			write_int (objectid);
			write_short (moveid);
			write_byte (pNibbles);
			write_byte (rawActivity);
			write_byte (skillId);
			write_byte (skillLv);
			write_short (pOption);
	...
	public class PickupItemPacket : OutPacket
	{
		...
		public PickupItemPacket (int oid, Point_short position) : base ((short)OutPacket.Opcode.PICKUP_ITEM)
		{
			write_int (0);
			write_byte (0);
			write_point (new Point_short (position));
			write_int (oid);
		}
	}
	...
	public class DamageReactorPacket : OutPacket
	{
		...
		public DamageReactorPacket (int oid, Point_short position, short stance, int skillid) : base ((short)OutPacket.Opcode.DAMAGE_REACTOR)
		{
			write_int (oid);
			write_point (new Point_short (position));
			write_short (stance);
