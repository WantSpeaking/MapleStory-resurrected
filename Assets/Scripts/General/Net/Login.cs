...
namespace ms
{
	...
	public class StatsEntry
	{
		...
		public StatsEntry (){}
		public StatsEntry (StatsEntry src)
		{
			name = src.name;
			female = src.female;
			foreach (var petid in src.petids)
			{
				petids.Add (petid);
			}

			foreach (var valuePair in src.stats)
			{
				stats.TryAdd (valuePair.Key, valuePair.Value);
			}

			exp = src.exp;
			mapid = src.mapid;
			portal = src.portal;
			rank = new Tuple<int, sbyte> (src.rank.Item1, src.rank.Item2);
			jobrank = new Tuple<int, sbyte> (src.jobrank.Item1, src.jobrank.Item2);
		}
	}
