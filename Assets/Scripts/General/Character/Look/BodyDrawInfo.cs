...
namespace ms
{
	...
	public class BodyDrawInfo
	{
		public void init ()
		{
			...
			if (node_Characterwz_00002000img is WzImage property_Characterwz_00002000img)
			{
				foreach (var property_Characterwz_00002000img_airstrike in property_Characterwz_00002000img.WzProperties)
				{
					...
					++frame)*/
						{
							if (!byte.TryParse (property_Characterwz_00002000img_fly_0.Name, out var frame))
							{
								continue;
							}
		...
		{
			init_Dict ();
			var node_Characterwz_00002000img = ms.wz.xml_character.getData("00002000.img");
			var node_Characterwz_00012000img = ms.wz.xml_character.getData("00012000.img");

			//if (node_Characterwz_00002000img is WzImage property_Characterwz_00002000img)
			{
				foreach (var property_Characterwz_00002000img_airstrike in node_Characterwz_00002000img)
				{
					string ststr = property_Characterwz_00002000img_airstrike.Name;
					...
					++frame)#1#
						{
							...
							else
							{
								...
								//if (stance == Stance.Id.STAND1)
								{
									...
									if (property_Characterwz_00002000img_fly_0?.Children != null) //todo 2 WzProperties == null?
									{
										foreach (var property_Characterwz_00002000img_fly_0_arm in property_Characterwz_00002000img_fly_0)
										{
											...
											if (part != "delay" && part != "face")
											{
												...
												foreach (var property_Characterwz_00002000img_fly_0_arm_hand in property_Characterwz_00002000img_fly_0_arm["map"])
												{
													...
													bodyshiftmap[z].TryAdd (property_Characterwz_00002000img_fly_0_arm_hand.Name, property_Characterwz_00002000img_fly_0_arm_hand, true);
												}
											}
										}
									}
									...
									if (node_Characterwz_00012000img_front_head_map is MapleData property_Characterwz_00012000img_front_head_map)
									{
										foreach (var property_Characterwz_00012000img_front_head_map_brow in property_Characterwz_00012000img_front_head_map)
										{
											//AppDebug.Log ($"{property_Characterwz_00012000img_front_head_map_brow.FullPath}");
											//bodyshiftmap.TryAdd (Body.Layer.HEAD);
											bodyshiftmap[Body.Layer.HEAD].TryAdd (property_Characterwz_00012000img_front_head_map_brow.Name, property_Characterwz_00012000img_front_head_map_brow, true);
										}
									}
								...
								head_positions[(int)stance][frame] = bodyshiftmap[Body.Layer.BODY]["neck"] - bodyshiftmap[Body.Layer.HEAD]["neck"];
								face_positions[(int)stance][frame] = bodyshiftmap[Body.Layer.BODY]["neck"] - bodyshiftmap[Body.Layer.HEAD]["neck"] + bodyshiftmap[Body.Layer.HEAD]["brow"];
								hair_positions[(int)stance][frame] = bodyshiftmap[Body.Layer.HEAD]["brow"] - bodyshiftmap[Body.Layer.HEAD]["neck"] + bodyshiftmap[Body.Layer.BODY]["neck"];#1#
							}
						}
					}
				}
