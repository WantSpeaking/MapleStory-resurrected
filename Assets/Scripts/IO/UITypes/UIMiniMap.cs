...
namespace ms
{
	public class UIMiniMap : UIDragElement<PosMINIMAP>
	{
		...
		public UIMiniMap (CharStats stats) : base (new Point<short> (128, 20))
		{
			...
			listNpc_dimensions = new Point<short> (150, 170);
			listNpc_offset = 0;
			selected = -1;
			...
			MiniMap = nl.nx.wzFile_ui["UIWindow2.img"][node];
			listNpc = nl.nx.wzFile_ui["UIWindow2.img"]["MiniMap"]["ListNpc"];

			buttons[(int)Buttons.BT_MIN] = new MapleButton (MiniMap["BtMin"], new Point<short> (195, -6));
			buttons[(int)Buttons.BT_MAX] = new MapleButton (MiniMap["BtMax"], new Point<short> (209, -6));
			buttons[(int)Buttons.BT_SMALL] = new MapleButton (MiniMap["BtSmall"], new Point<short> (223, -6));
			buttons[(int)Buttons.BT_BIG] = new MapleButton (MiniMap["BtBig"], new Point<short> (223, -6));
			buttons[(int)Buttons.BT_MAP] = new MapleButton (MiniMap["BtMap"], new Point<short> (237, -6));
			buttons[(int)Buttons.BT_NPC] = new MapleButton (MiniMap["BtNpc"], new Point<short> (276, -6));
...
//ORIGINAL LINE: sprite.draw(position, alpha);
				...
				combined_text.draw (position + new Point<short> (7, -3));
			}
...
//ORIGINAL LINE: sprite.draw(position, alpha);
				...
				{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Animation portal_marker = Animation(marker["portal"]);
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: draw_movable_markers(position, alpha);
					draw_movable_markers (position, alpha);

					if (listNpc_enabled)
					{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: draw_npclist(normal_dimensions, alpha);
...
//ORIGINAL LINE: sprite.draw(position, alpha);
				...
				region_text.draw (position + new Point<short> (48, 14));
				town_text.draw (position + new Point<short> (48, 28));

				if (has_map)
				{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Animation portal_marker(marker["portal"]);
					Animation portal_marker = new Animation (marker["portal"]);

					foreach (var sprite in static_marker_info)
					{
						portal_marker.draw (position + sprite.Item2 + new Point<short> (0, MAX_ADJ), alpha);
					}

					draw_movable_markers (position + new Point<short> (0, MAX_ADJ), alpha);

					if (listNpc_enabled)
					{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: draw_npclist(max_dimensions, alpha);
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: center_offset = Point<short>(miniMap["centerX"], miniMap["centerY"]);
				center_offset = new Point<short> (miniMap["centerX"], miniMap["centerY"]);
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: if (Cursor::State new_state = listNpc_slider.send_cursor(cursor_relative, clicked))
				Cursor.State new_state = listNpc_slider.send_cursor (cursor_relative, clicked);
			...
			if (listNpc_enabled)
			{
				Point<short> relative_point = cursor_relative - new Point<short> ((short)(10 + (type == (int)((int)Type.MAX) ? max_dimensions : normal_dimensions).x ()), 23);
				Rectangle<short> list_bounds = new Rectangle<short> (0, LISTNPC_ITEM_WIDTH, 0, LISTNPC_ITEM_HEIGHT * 8);
				...
				if (list_bounds.contains (relative_point))
				{
					...
					{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: UI::get().show_text(Tooltip::Parent::MINIMAP, listNpc_full_names[list_index]);
			...
			{
				Point<short> npc_pos = (npc.Value.get_position () + center_offset) / scale + new Point<short> (map_draw_origin_x, map_draw_origin_y);
				Rectangle<short> marker_spot = new Rectangle<short> (npc_pos - new Point<short> (4, 8), npc_pos);

				if (type == (int)Type.MAX)
				{
					marker_spot.shift (new Point<short> (0, MAX_ADJ));
				}
			...
			if (!found)
			{
				foreach (var sprite in static_marker_info)
				{
					Rectangle<short> marker_spot = new Rectangle<short> (sprite.Item2, sprite.Item2 + (short)8);
					...
					if (marker_spot.contains (cursor_relative))
					{
						...
						WzObject portal_name = nl.nx.wzFile_string["Map.img"][portal_cat][portal_tm.ToString ()]["mapName"];
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: UI::get().show_map(Tooltip::Parent::MINIMAP, portal_name, "", portal_tm, false);
...
//ORIGINAL LINE: UIElement::Type get_type() const override
		...
		private static readonly Point<short> WINDOW_UL_POS = Point<short>.zero;
		...
		private void toggle_buttons ()
		{
			...
			if (type == (int)Type.MIN)
			{
				...
				buttons[(int)Buttons.BT_MIN].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));

				bt_min_x += bt_min_width;

				buttons[(int)Buttons.BT_MAX].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));

				bt_min_x += bt_max_width;

				buttons[(int)Buttons.BT_MAP].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));

				min_dimensions = new Point<short> ((short)(bt_min_x + bt_map_width + 7), 20);
...
//ORIGINAL LINE: dragarea = dimension;
				dragarea = (dimension);
			...
			else
			{
				...
				buttons[(int)Buttons.BT_SMALL].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));
				buttons[(int)Buttons.BT_BIG].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));
				...
				buttons[(int)Buttons.BT_NPC].set_position (new Point<short> (bt_min_x, BTN_MIN_Y));
		...
		private void update_canvas ()
		{
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Min = MiniMap["Window"]["Min"];
				Min = (MiniMap["Window"]["Min"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Normal = MiniMap["Window"]["Normal"];
				Normal = (MiniMap["Window"]["Normal"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Max = MiniMap["Window"]["Max"];
				Max = (MiniMap["Window"]["Max"]);
			}
			else
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Min = MiniMap["Min"];
				Min = (MiniMap["Min"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Normal = MiniMap["MinMap"];
				Normal = (MiniMap["MinMap"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: Max = MiniMap["MaxMap"];
				Max = (MiniMap["MaxMap"]);
			}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: map_sprite = Texture(Map["miniMap"]["canvas"]);
			map_sprite = new Texture (Map["miniMap"]["canvas"]);
			Point<short> map_dimensions = map_sprite.get_dimensions ();
			...
			// Min sprites queue
			min_sprites.Add (new Sprite (Min[Center], new DrawArgument (WINDOW_UL_POS + new Point<short> (CENTER_START_X, 0), new Point<short> (min_c_stretch, 0))));
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: min_sprites.Add(Min[Left], DrawArgument(WINDOW_UL_POS));
			min_sprites.Add (new Sprite (Min[Left], new DrawArgument (WINDOW_UL_POS)));
			min_sprites.Add (new Sprite (Min[Right], new DrawArgument (WINDOW_UL_POS + new Point<short> ((short)(min_c_stretch + CENTER_START_X), 0))));
			...
			normal_sprites.Add (new Sprite (MiddleCenter, new DrawArgument (new Point<short> (7, 10), new Point<short> ((short)(c_stretch + 114), (short)(m_stretch + 27)))));

			if (has_map)
			{
				normal_sprites.Add (new Sprite (Map["miniMap"]["canvas"], new DrawArgument (new Point<short> (map_draw_origin_x, map_draw_origin_y))));
			}

			normal_sprites.Add (new Sprite (Normal[MiddleLeft], new DrawArgument (new Point<short> (0, ML_MR_Y), new Point<short> (0, m_stretch))));
			normal_sprites.Add (new Sprite (Normal[MiddleRight], new DrawArgument (new Point<short> (middle_right_x, ML_MR_Y), new Point<short> (0, m_stretch))));
			normal_sprites.Add (new Sprite (Normal[UpCenter], new DrawArgument (new Point<short> (CENTER_START_X, 0) + WINDOW_UL_POS, new Point<short> (c_stretch, 0))));
			normal_sprites.Add (new Sprite (Normal[UpLeft], WINDOW_UL_POS));
			normal_sprites.Add (new Sprite (Normal[UpRight], new DrawArgument (new Point<short> (ur_x_offset, 0) + WINDOW_UL_POS)));
			normal_sprites.Add (new Sprite (Normal[DownCenter], new DrawArgument (new Point<short> (CENTER_START_X, (short)(down_y_offset + 18)), new Point<short> (c_stretch, 0))));
			normal_sprites.Add (new Sprite (Normal[DownLeft], new Point<short> (0, down_y_offset)));
			normal_sprites.Add (new Sprite (Normal[DownRight], new Point<short> (ur_x_offset, down_y_offset)));

			normal_dimensions = new Point<short> ((short)(ur_x_offset + 64), (short)(down_y_offset + 27));

			// Max sprites queue
			max_sprites.Add (new Sprite (MiddleCenter, new DrawArgument (new Point<short> (7, 50), new Point<short> ((short)(c_stretch + 114), (short)(m_stretch + 27)))));

			if (has_map)
			{
				max_sprites.Add (new Sprite (Map["miniMap"]["canvas"], new DrawArgument (new Point<short> (map_draw_origin_x, (short)(map_draw_origin_y + MAX_ADJ)))));
			}

			max_sprites.Add (new Sprite (Max[MiddleLeft], new DrawArgument (new Point<short> (0, ML_MR_Y + MAX_ADJ), new Point<short> (0, m_stretch))));
			max_sprites.Add (new Sprite (Max[MiddleRight], new DrawArgument (new Point<short> (middle_right_x, ML_MR_Y + MAX_ADJ), new Point<short> (0, m_stretch))));
			max_sprites.Add (new Sprite (Max[UpCenter], new DrawArgument (new Point<short> (CENTER_START_X, 0) + WINDOW_UL_POS, new Point<short> (c_stretch, 0))));
			max_sprites.Add (new Sprite (Max[UpLeft], WINDOW_UL_POS));
			max_sprites.Add (new Sprite (Max[UpRight], new DrawArgument (new Point<short> (ur_x_offset, 0) + WINDOW_UL_POS)));
			max_sprites.Add (new Sprite (Max[DownCenter], new DrawArgument (new Point<short> (CENTER_START_X, (short)(down_y_offset + MAX_ADJ + 18)), new Point<short> (c_stretch, 0))));
			max_sprites.Add (new Sprite (Max[DownLeft], new Point<short> (0, (short)(down_y_offset + MAX_ADJ))));
			max_sprites.Add (new Sprite (Max[DownRight], new Point<short> (ur_x_offset, (short)(down_y_offset + MAX_ADJ))));
			max_sprites.Add (new Sprite (nl.nx.wzFile_mapLatest["MapHelper.img"]["mark"][Map["info"]["mapMark"].ToString ()], new DrawArgument (new Point<short> (7, 17))));

			max_dimensions = normal_dimensions + new Point<short> (0, MAX_ADJ);
		}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: void draw_movable_markers(Point<short> init_pos, float alpha) const
		private void draw_movable_markers (Point<short> init_pos, float alpha)
		{
			if (!has_map)
			{
				return;
			...
			Point<short> sprite_offset = new Point<short> ();

			/// NPCs
			MapObjects npcs = Stage.get ().get_npcs ().get_npcs ();
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: marker_sprite = Animation(marker["npc"]);
			marker_sprite = new Animation (marker["npc"]);
			sprite_offset = marker_sprite.get_dimensions () / new Point<short> (2, 0);

			foreach (var npc in npcs)
			{
				Point<short> npc_pos = npc.Value.get_position ();
				marker_sprite.draw ((npc_pos + center_offset) / scale - sprite_offset + new Point<short> (map_draw_origin_x, map_draw_origin_y) + init_pos, alpha);
			}
			...
			{
				Point<short> chr_pos = chr.Value.get_position ();
				marker_sprite.draw ((chr_pos + center_offset) / scale - sprite_offset + new Point<short> (map_draw_origin_x, map_draw_origin_y) + init_pos, alpha);
			}

			/// Player
			Point<short> player_pos = Stage.get ().get_player ().get_position ();
			sprite_offset = player_marker.get_dimensions () / new Point<short> (2, 0);
			player_marker.draw ((player_pos + center_offset) / scale - sprite_offset + new Point<short> (map_draw_origin_x, map_draw_origin_y) + init_pos, alpha);
		}
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: marker_sprite = Animation(marker["portal"]);
			marker_sprite = new Animation (marker["portal"]);
			Point<short> marker_offset = marker_sprite.get_dimensions () / new Point<short> (2, 0);
			foreach (var portal in portals)
			{
				...
				if (portal_type == 2)
				{
					Point<short> marker_pos = (new Point<short> (portal["x"], portal["y"]) + center_offset) / scale - marker_offset + new Point<short> (map_draw_origin_x, map_draw_origin_y);
					static_marker_info.Add (new Tuple<string, Point<short>> (portal.Name, marker_pos));
				}
			}
		}
		...
		private void update_dimensions ()
		{
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: dimension = min_dimensions;
				dimension = (min_dimensions);
			}
			else
			{
				Point<short> base_dims = type == (int)((int)Type.MAX) ? max_dimensions : normal_dimensions;
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: dimension = base_dims;
				dimension = (base_dims);
		...
		private void update_npclist ()
		{
			...
			Point<short> listNpc_pos = new Point<short> (type == (int)((int)Type.MAX) ? max_dimensions.x () : normal_dimensions.x (), 0);
			short c_stretch = 20;
			short m_stretch = 102;

			if (listNpc_names.Count > 8)
			{
				listNpc_slider = new Slider ((int)Slider.Type.DEFAULT_SILVER, new Range<short> (23, 11 + LISTNPC_ITEM_HEIGHT * 8), (short)(listNpc_pos.x () + LISTNPC_ITEM_WIDTH + 1), 8, (short)listNpc_names.Count, (bool upwards) =>
				{
			...
			listNpc_sprites.Add (new Sprite (listNpc["c"], new DrawArgument (listNpc_pos + new Point<short> (CENTER_START_X, M_START), new Point<short> (c_stretch, m_stretch))));
			listNpc_sprites.Add (new Sprite (listNpc["w"], new DrawArgument (listNpc_pos + new Point<short> (0, M_START), new Point<short> (0, m_stretch))));
			listNpc_sprites.Add (new Sprite (listNpc["e"], new DrawArgument (listNpc_pos + new Point<short> ((short)(CENTER_START_X + c_stretch), M_START), new Point<short> (0, m_stretch))));
			listNpc_sprites.Add (new Sprite (listNpc["n"], new DrawArgument (listNpc_pos + new Point<short> (CENTER_START_X, 0), new Point<short> (c_stretch, 0))));
			listNpc_sprites.Add (new Sprite (listNpc["s"], new DrawArgument (listNpc_pos + new Point<short> (CENTER_START_X, (short)(M_START + m_stretch)), new Point<short> (c_stretch, 0))));
			listNpc_sprites.Add (new Sprite (listNpc["nw"], new DrawArgument (listNpc_pos + new Point<short> (0, 0))));
			listNpc_sprites.Add (new Sprite (listNpc["ne"], new DrawArgument (listNpc_pos + new Point<short> ((short)(CENTER_START_X + c_stretch), 0))));
			listNpc_sprites.Add (new Sprite (listNpc["sw"], new DrawArgument (listNpc_pos + new Point<short> (0, (short)(M_START + m_stretch)))));
			listNpc_sprites.Add (new Sprite (listNpc["se"], new DrawArgument (listNpc_pos + new Point<short> ((short)(CENTER_START_X + c_stretch), (short)(M_START + m_stretch)))));

			listNpc_dimensions = new Point<short> ((short)(CENTER_START_X * 2 + c_stretch), (short)(M_START + m_stretch + 30));

			update_dimensions ();
		}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: void draw_npclist(Point<short> minimap_dims, float alpha) const
		private void draw_npclist (Point<short> minimap_dims, float alpha)
		{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Animation npc_marker = Animation(marker["npc"]);
...
//ORIGINAL LINE: sprite.draw(position, alpha);
			...
			Point<short> listNpc_pos = position + new Point<short> ((short)(minimap_dims.x () + 10), 23);
			...
			for (sbyte i = 0; i + listNpc_offset < listNpc_list.Count && i < 8; i++)
			{
				...
				npc_marker.draw (new DrawArgument (listNpc_pos + new Point<short> (0, 2), false, npc_marker.get_dimensions () / 2), alpha);
				listNpc_names[listNpc_offset + i].draw (new DrawArgument (listNpc_pos + new Point<short> (14, -2)));
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: listNpc_slider.draw(position);
				listNpc_slider.draw (position);
			}

			if (selected >= 0)
			{
				Point<short> npc_pos = (listNpc_list[selected].get_position () + center_offset) / scale + new Point<short> (map_draw_origin_x, (short)(map_draw_origin_y - npc_marker.get_dimensions ().y () + (type == (int)((int)Type.MAX) ? MAX_ADJ : 0)));
		...
		private List<System.Tuple<string, Point<short>>> static_marker_info = new List<System.Tuple<string, Point<short>>> ();
		private short map_draw_origin_x;
		private short map_draw_origin_y;
		private Point<short> center_offset = new Point<short> ();
		private Point<short> min_dimensions = new Point<short> ();
		private Point<short> normal_dimensions = new Point<short> ();
		private Point<short> max_dimensions = new Point<short> ();
		...
		private Point<short> listNpc_dimensions = new Point<short> ();
