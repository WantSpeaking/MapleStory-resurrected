...
namespace ms
{
	public class UISkillBook : UIDragElement<PosSKILL>
	{
		...
		public UISkillBook(CharStats in_stats, SkillBook in_skillbook)
		{
			...
			WzObject Skill = nl.nx.wzFile_ui["UIWindow2.img"]["Skill"];
			...
			WzObject skillPoint = nl.nx.wzFile_ui["UIWindow4.img"]["Skill"]["skillPoint"];

			sp_backgrnd = skillPoint["backgrnd"];
			sp_backgrnd2 = skillPoint["backgrnd2"];
			sp_backgrnd3 = skillPoint["backgrnd3"];

			buttons[(int)Buttons.BT_CANCLE] = new MapleButton(skillPoint["BtCancle"], new Point<short>(bg_dimensions.x(), 0));
			buttons[(int)Buttons.BT_OKAY] = new MapleButton(skillPoint["BtOkay"], new Point<short>(bg_dimensions.x(), 0));
			buttons[(int)Buttons.BT_SPDOWN] = new MapleButton(skillPoint["BtSpDown"], new Point<short>(bg_dimensions.x(), 0));
			buttons[(int)Buttons.BT_SPMAX] = new MapleButton(skillPoint["BtSpMax"], new Point<short>(bg_dimensions.x(), 0));
			buttons[(int)Buttons.BT_SPUP] = new MapleButton(skillPoint["BtSpUp"], new Point<short>(bg_dimensions.x(), 0));

			buttons[(int)Buttons.BT_SPDOWN].set_state(Button.State.DISABLED);

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: sp_before = Charset(skillPoint["num"], Charset::Alignment::RIGHT);
			sp_before = new Charset(skillPoint["num"], Charset.Alignment.RIGHT);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: sp_after = Charset(skillPoint["num"], Charset::Alignment::RIGHT);
			...
			sprites.Add(new Sprite (ui_backgrnd, new Point<short>(1, 0)));
			...
			buttons[(int)Buttons.BT_MACRO_OK] = new MapleButton(macro["BtOK"], new Point<short>(bg_dimensions.x(), 0));

			buttons[(int)Buttons.BT_MACRO_OK].set_state(Button.State.DISABLED);

			WzObject close = nl.nx.wzFile_ui["Basic.img"]["BtClose3"];

			buttons[(int)Buttons.BT_CLOSE] = new MapleButton(close, new Point<short>((short)(bg_dimensions.x() - 23), 6));
			...
			for (ushort i = (int)Buttons.BT_SPUP0; i <= (int)Buttons.BT_SPUP11; ++i)
			{
				...
				Point<short> spup_position = SKILL_OFFSET + new Point<short>((short)(124 + x_adj), (short)(20 + y_adj));
			...
			slider = new Slider((int)Slider.Type.DEFAULT_SILVER, new Range<short>(93, 317), 295, ROWS, 1, (bool upwards) =>
			{
...
//ORIGINAL LINE: void draw(float alpha) const override
		public override void draw(float alpha)
		{
			...
			bookicon.draw(position + new Point<short>(11, 85));
			booktext.draw(position + new Point<short>(173, 59));
			splabel.draw(position + new Point<short>(304, 23));

			Point<short> skill_position_l = position + SKILL_OFFSET + new Point<short>(-1, 0);
			Point<short> skill_position_r = position + SKILL_OFFSET + new Point<short>(-1 + ROW_WIDTH, 0);

			for (int i = 0; i < ROWS; i++)
			{
				Point<short> pos = skill_position_l;
...
//ORIGINAL LINE: slider.draw(position);
			...
			if (macro_enabled)
			{
				Point<short> macro_pos = position + new Point<short>(bg_dimensions.x(), 0);

				macro_backgrnd.draw(macro_pos + new Point<short>(1, 0));
			...
			if (sp_enabled)
			{
				Point<short> sp_pos = position + new Point<short>(bg_dimensions.x(), 0);

				sp_backgrnd.draw(sp_pos);
				sp_backgrnd2.draw(sp_pos);
				sp_backgrnd3.draw(sp_pos);

				Point<short> sp_level_pos = sp_pos + new Point<short>(78, 149);

				sp_before.draw(sp_before_text, 12, sp_level_pos);
				sp_after.draw(sp_after_text, 11, sp_level_pos + new Point<short>(78, 0));
				sp_used.draw(sp_pos + new Point<short>(82, 87));
				sp_remaining.draw(sp_pos + new Point<short>(76, 65));
				sp_name.draw(sp_pos + new Point<short>(97, 35));
				sp_skill.draw(sp_pos + new Point<short>(13, 31));
			}
		...
		static readonly Rectangle<short> bounds = new Rectangle<short>(0, 32, 0, 32);
...
//ORIGINAL LINE: Cursor::State dstate = UIDragElement::send_cursor(clicked, cursorpos);
			...
			if (slider.isenabled())
			{
				Cursor.State new_state = slider.send_cursor (cursor_relative, clicked);
			...
			if (!grabbing)
			{
				...
				{
					Point<short> skill_position = skill_position_l;

					if (i % 2 != 0)
					{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: skill_position = skill_position_r;
...
//ORIGINAL LINE: void drop_on_bindings(Point<short> cursorposition, bool remove) const override
		...
		private class SkillDisplayMeta
		{
			public SkillDisplayMeta(int i, int l)
			{
				...
				{
					namestr = namestr.Remove(namestr.end() - overhang, namestr.end()).Insert(namestr.end() - overhang, "..");
					overhang += 1;
...
//ORIGINAL LINE: void draw(const DrawArgument& args) const
			public void draw(DrawArgument args)
			{
				...
				name_text.draw(args + new Point<short>(38, -5));
				level_text.draw(args + new Point<short>(38, 13));
			}
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: int get_level() const
			public int get_level()
			{
				return level;
			}
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: StatefulIcon* get_icon() const
			...
			private int id;
			private int level;
			private StatefulIcon icon = new StatefulIcon();
			private Text name_text = new Text();
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: bookicon = data.get_icon();
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool can_raise(int skill_id) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: sp_skill = skills[row].get_icon()->get_texture();
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: Job::Level joblevel_by_tab(ushort t) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: const UISkillBook::SkillDisplayMeta* skill_by_position(Point<short> cursorpos) const
		private UISkillBook.SkillDisplayMeta skill_by_position(Point<short> cursorpos)
		{
			short x = cursorpos.x();
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool check_required(int id) const
		...
		private void set_macro(bool enabled)
		{
			...
			if (macro_enabled)
			{
				dimension = bg_dimensions + new Point<short>(macro_backgrnd.get_dimensions().x(), 0);
			}
			else if (!sp_enabled)
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: dimension = bg_dimensions;
		...
		private void set_skillpoint(bool enabled)
		{
			...
			if (sp_enabled)
			{
				dimension = bg_dimensions + new Point<short>(sp_backgrnd.get_dimensions().x(), 0);
			}
...
//ORIGINAL LINE: dimension = bg_dimensions;
		...
		private static readonly Point<short> SKILL_OFFSET = new Point<short>(11, 93);
		private static readonly  Point<short> SKILL_META_OFFSET = new Point<short>(2, 2);
		private static readonly  Point<short> LINE_OFFSET = new Point<short>(0, 37);
		...
		private Point<short> bg_dimensions = new Point<short>();
