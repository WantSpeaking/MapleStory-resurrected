...
namespace ms
{
	...
	public class UIItemInventory : UIDragElement<PosINV>
	{
		...
		public UIItemInventory (Inventory invent)
		{
			...
			WzObject Item = nl.nx.wzFile_ui["UIWindow2.img"]["Item"];
...
//ORIGINAL LINE: bg_dimensions = backgrnd.get_dimensions();
			bg_dimensions = (backgrnd.get_dimensions ());
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: bg_full_dimensions = full_backgrnd.get_dimensions();
			...
			slider = new Slider ((int)Slider.Type.DEFAULT_SILVER, new Range<short> (50, 245), 152, 6, rm: (short)(1 + inventory.get_slotmax (tab) / COLUMNS), (bool upwards) =>
			{
...
//ORIGINAL LINE: void draw(float alpha) const override
		public override void draw (float alpha)
		{
			...
			Point<short> mesolabel_pos = position + new Point<short> (127, 262);
			Point<short> maplepointslabel_pos = position + new Point<short> (159, 279);
			...
			if (full_enabled)
			{
				...
				mesolabel.draw (mesolabel_pos + new Point<short> (3, 70));
				maplepointslabel.draw (maplepointslabel_pos + new Point<short> (181, 53));
			}
			else
			{
				...
				slider.draw (position + new Point<short> (0, 1));
			...
			{
				Point<short> slotpos = full_enabled ? get_full_slotpos ((short)i) : get_slotpos (i);
			...
			if (tab == newtab)
			{
				...
				{
					newitemslot.draw (position + get_slotpos (newslot) + new Point<short> (1, 1), alpha);
				}
			}
...
//ORIGINAL LINE: Cursor::State dstate = UIDragElement::send_cursor(pressed, cursorpos);
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Cursor::State sstate = slider.send_cursor(cursor_relative, pressed);
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: short slot = slot_by_position(cursor_relative);
			short slot = slot_by_position (cursor_relative);
			...
			if (is_icon)
			{
				if (pressed)
				{
					Point<short> slotpos = get_slotpos (slot);
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: return UIElement::send_cursor(pressed, cursorpos);
				return base.send_cursor (pressed, cursorpos);
			}
		}
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool is_visible(short slot) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool is_not_visible(short slot) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: bool can_wear_equip(short slot) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: short slot_by_position(Point<short> cursorpos) const
		private short slot_by_position (Point<short> cursorpos)
		{
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: ushort button_by_tab(InventoryType::Id tb) const
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: Point<short> get_slotpos(short slot) const
		private Point<short> get_slotpos (short slot)
		{
			short absslot = (short)(slot - slotrange[tab].Item1);

			return new Point<short> ((short)(10 + (absslot % COLUMNS) * ICON_WIDTH), (short)(51 + (absslot / COLUMNS) * ICON_HEIGHT));
		}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: Point<short> get_full_slotpos(short slot) const
		private Point<short> get_full_slotpos (short slot)
		{
			...
			return new Point<short> ((short)(10 + adj_x + (new_slot % COLUMNS) * ICON_WIDTH), (short)(51 + (new_slot / COLUMNS) * ICON_HEIGHT));
		}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: Point<short> get_tabpos(InventoryType::Id tb) const
		private Point<short> get_tabpos (InventoryType.Id tb)
		{
			...
			return new Point<short> ((short)(10 + ((fixed_tab - 1) * 31)), 29);
		}
		...
		private void set_full (bool enabled)
		{
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: dimension = bg_full_dimensions;
...
//ORIGINAL LINE: dimension = bg_dimensions;
			...
			buttons[(int)Buttons.BT_CLOSE].set_position (new Point<short> ((short)(dimension.x () - adj_x), 6));
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: void drop_on_equips(EquipSlot::Id eqslot) const override
...
//ORIGINAL LINE: void drop_on_bindings(Point<short> cursorposition, bool remove) const override
		...
		private Texture backgrnd = new Texture ();
		private Texture backgrnd2 = new Texture ();
		private Texture backgrnd3 = new Texture ();
		private Texture full_backgrnd = new Texture ();
		private Texture full_backgrnd2 = new Texture ();
		private Texture full_backgrnd3 = new Texture ();
		private Point<short> bg_dimensions = new Point<short> ();
		private Point<short> bg_full_dimensions = new Point<short> ();
	}
}
