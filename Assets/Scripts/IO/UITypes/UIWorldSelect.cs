...
namespace ms
{
	public class UIWorldSelect : UIElement
	{
		...
		public UIWorldSelect () : base (new Point<short> (0, 0), new Point<short> (800, 600))
		{
			...
			Point<short> background_pos = new Point<short> (400, 301);
			channelsrc_pos = new Point<short> (203, 164);
			...
			WzObject obj = nl.nx.wzFile_mapLatest["Obj"]["login.img"];
...
//ORIGINAL LINE: worldselect = login["WorldSelect"];
			worldselect = (login["WorldSelect"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: worldsrc = worldselect["BtWorld"]["release"];
			worldsrc = (worldselect["BtWorld"]["release"]);
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy assignment (rather than a reference assignment) - this should be verified and a 'CopyFrom' method should be created if it does not yet exist:
//ORIGINAL LINE: channelsrc = worldselect["BtChannel"];
			...
			sprites.Add (new Sprite (common["step"]["1"], new Point<short> (40, 0)));

			buttons[(int)Buttons.BT_VIEWALL] = new MapleButton (worldselect["BtViewAll"], new Point<short> (0, 53));
			buttons[(int)Buttons.BT_VIEWRECOMMENDED] = new MapleButton (worldselect["BtViewChoice"], new Point<short> (0, 53));
			buttons[(int)Buttons.BT_VIEWRECOMMENDED_SELECT] = new MapleButton (worldselect["alert"]["BtChoice"], new Point<short> (349, 327));
			buttons[(int)Buttons.BT_VIEWRECOMMENDED_CANCEL] = new MapleButton (worldselect["alert"]["BtClose"], new Point<short> (407, 327));
			buttons[(int)Buttons.BT_VIEWRECOMMENDED_PREV] = new MapleButton (worldselect["alert"]["BtArrowL"], new Point<short> (338, 244));
			buttons[(int)Buttons.BT_VIEWRECOMMENDED_NEXT] = new MapleButton (worldselect["alert"]["BtArrowR"], new Point<short> (439, 244));
			...
			buttons[(int)Buttons.BT_CHANGEREGION] = new MapleButton (worldselect["BtRegion"], new Point<short> (3, 127));
			buttons[(int)Buttons.BT_QUITGAME] = new MapleButton (common["BtExit"], new Point<short> (0, 515));
...
//ORIGINAL LINE: void draw(float alpha) const override
		public override void draw (float alpha)
		{
			...
			if (show_recommended)
			{
				recommended_textures[0].draw (position + new Point<short> (302, 152));
				recommended_world_textures[recommended_worldid].draw (position + new Point<short> (336, 187));
				recommended_message.draw (position + new Point<short> (401, 259));
			}
			...
			if (draw_chatballoon)
			{
				chatballoon.draw (position + new Point<short> (501, 105));
			}
		}

		public override Cursor.State send_cursor (bool clicked, Point<short> cursorpos)
		{
			Rectangle<short> channels_bounds = new Rectangle<short> (position + channelsrc_pos, position + channelsrc_pos + channels_background.get_dimensions ());

			Rectangle<short> worlds_bounds = new Rectangle<short> (position + worldsrc_pos, position + worldsrc_pos + worlds_background.get_dimensions ());
...
//ORIGINAL LINE: buttons[Buttons::BT_WORLD0 + i] = <TwoSpriteButton>(worldbtn["normal"]["0"], worldbtn["keyFocused"]["0"], worldsrc_pos + Point<short>(region["origin"][i + 1]));
		...
		private Point<short> worldsrc_pos = new Point<short> ();
		private Point<short> channelsrc_pos = new Point<short> ();
