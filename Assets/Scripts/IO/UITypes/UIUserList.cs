...
namespace ms
{
	public class UIUserList : UIDragElement<PosUSERLIST>
	{
		...
		public UIUserList (ushort t) : base (new Point<short> (260, 20))
		{
			...
			UserList = nl.nx.wzFile_ui["UIWindow2.img"]["UserList"];
			WzObject Main = UserList["Main"];

			sprites.Add (Main["backgrnd"]);

			buttons[(int)Buttons.BT_CLOSE] = new MapleButton (close, new Point<short> (244, 7));
			...
			party_slider = new Slider ((int)Slider.Type.DEFAULT_SILVER, new Range<short> (party_y, party_height), party_x, party_unitrows, party_rowmax, (bool UnnamedParameter1) => { });
			...
			friends_slider = new Slider ((int)Slider.Type.DEFAULT_SILVER, new Range<short> (friends_y, friends_height), friends_x, friends_unitrows, friends_rowmax, (bool UnnamedParameter1) => { });
			...
			dimension = new Point<short> (276, 390);
		}
...
//ORIGINAL LINE: void draw(float alpha) const override
		public override void draw (float alpha)
		{
			...
			if (tab == (int)Buttons.BT_TAB_PARTY)
			{
				...
				{
					party_mine_grid[0].draw (position + new Point<short> (10, 115));
					party_mine_grid[4].draw (position + new Point<short> (10, 133));
					party_mine_name.draw (position + new Point<short> (27, 130));
				}
...
//ORIGINAL LINE: party_slider.draw(position);
...
//ORIGINAL LINE: sprite.draw(position, alpha);
				...
				friends_online_text.draw (position + new Point<short> (211, 62));
				friends_cur_location.draw (position + new Point<short> (9, 279));
				friend_grid[0].draw (position + new Point<short> (10, 116));
				friend_grid[2].draw (position + new Point<short> (10, 135));
				friends_name.draw (position + new Point<short> (24, 134));
				friends_group_name.draw (position + new Point<short> (29, 114));
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: friends_slider.draw(position);
...
//ORIGINAL LINE: sprite.draw(position, alpha);
			...
			{
				blacklist_title.draw (position + new Point<short> (24, 104));
				blacklist_grid[0].draw (position + new Point<short> (24, 134));
				blacklist_name.draw (position + new Point<short> (24, 134));
			}
