...
namespace ms
{
	public class UITermsOfService : UIElement
	{
		public const Type TYPE = UIElement.Type.TOS;
		...
		public UITermsOfService(System.Action oh)
		{
			...
			sprites.Add(new Sprite (TOS, new Point<short>(399, 250)));

			buttons[(int)Buttons.OK] = new MapleButton(Login["BtOk"], new Point<short>(483, 425));
			buttons[(int)Buttons.CANCEL] = new MapleButton(Login["BtCancel"], new Point<short>(557, 425));

			string linebreak = "\\r\\n";

#region EULA
			string EULA = "NEXON AMERICA, INC. - TERMS OF USE";
			...
			EULA += "manual or varmatic device or process to retrieve, index, \"data mine\" ";
			...
			EULA += "var-looting or robot play (i.e., \"botting\") or any other behavior that ";
			EULA += "allows you (or any character you are controlling) to varmatically ";
			...
			EULA += "codes (including but not limited to disconnection hacks), var clickers, ";
			...
			text = new Text(Text.Font.A11M, Text.Alignment.LEFT, Color.Name.BLACK, EULA, 340, true, 2);
			max_rows = (short)(text.height() / 300 + 1);

			short slider_y = 77;

			slider = new Slider((int)Slider.Type.LINE_PUNGA, new Range<short>(slider_y, (short)(slider_y + 305)), 574, unit_rows, max_rows, (bool upwards) =>
			{
			...
			position = new Point<short>(0, 10);
			dimension = new Texture(TOS).get_dimensions();
		}
...
//ORIGINAL LINE: void draw(float inter) const override
		public override void draw(float inter)
		{
			...
			short range_min = 80;
			text.draw(position + new Point<short>(226, (short)(84 - offset * 300)), new Range<short>(range_min, (short)(range_min + 316)));
...
//ORIGINAL LINE: slider.draw(position);
		...
		public override Cursor.State send_cursor(bool clicked, Point<short> cursorpos)
		{
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Cursor::State state = slider.send_cursor(cursoroffset, clicked);
...
//ORIGINAL LINE: UIElement::Type get_type() const override
		...
		private void update_accept(ushort offset)
		{
			...
			{
				buttons[(int)Buttons.OK].set_state(Button.State.DISABLED);
			}
		}
		...
		private short unit_rows;
		private short max_rows;
	}
}
