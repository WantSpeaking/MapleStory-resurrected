...
namespace ms
{
	public class UIWorldMap : UIDragElement<PosMAP>
	{
		...
		public UIWorldMap ()
		{
			...
			WzObject WorldMap = nl.nx.wzFile_ui["UIWindow2.img"]["WorldMap"];
			...
			WzObject MapHelper = nl.nx.wzFile_map["MapHelper.img"]["worldMap"];
...
//ORIGINAL LINE: bg_search_dimensions = search_background.get_dimensions();
			bg_search_dimensions = (search_background.get_dimensions ());

			short bg_dimension_x = bg_dimensions.x ();
			background_dimensions = new Point<short> (bg_dimension_x, 0);

			short base_x = (short)(bg_dimension_x / 2);
			short base_y = (short)(bg_dimensions.y () / 2);
			base_position = new Point<short> (base_x, (short)(base_y + 15));

			Point<short> close_dimensions = new Point<short> ((short)(bg_dimension_x - 22), 4);
			...
			buttons[(int)Buttons.BT_SEARCH_CLOSE] = new MapleButton (close, close_dimensions + new Point<short> (bg_search_dimensions.x (), 0));
			buttons[(int)Buttons.BT_ALLSEARCH] = new MapleButton (WorldMapSearch["BtAllsearch"], background_dimensions);

			Point<short> search_text_pos = new Point<short> ((short)(bg_dimension_x + 14), 25);
			Point<short> search_box_dim = new Point<short> (83, 15);
			Rectangle<short> search_text_dim = new Rectangle<short> (search_text_pos, search_text_pos + search_box_dim);

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: search_text = Textfield(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::BLACK, search_text_dim, 8);
			search_text = new Textfield (Text.Font.A11M, Text.Alignment.LEFT, Color.Name.BLACK, search_text_dim, 8);

			set_search (true);

			dragarea = new Point<short> (bg_dimension_x, 20);
		}
...
//ORIGINAL LINE: void draw(float alpha) const override
		public override void draw (float alpha)
		{
			...
			if (search)
			{
				...
				search_text.draw (position + new Point<short> (1, -5));
			}
		...
		public override void update ()
		{
			...
			{
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: search_text.update(position);
		...
		{
			Cursor.State new_state = search_text.send_cursor (cursorpos, clicked);
			...
			{
				Point<short> p = path.Item1 + position + base_position - (short)10;
				Point<short> d = p + path.Item2.marker.get_dimensions ();
				Rectangle<short> abs_bounds = new Rectangle<short> (p, d);
...
//ORIGINAL LINE: UIElement::Type get_type() const override
		...
		private void set_search (bool enable)
		{
			...
			if (enable)
			{
				...
				dimension = bg_dimensions + new Point<short> (bg_search_dimensions.x (), 0);
			}
			else
			{
...
//ORIGINAL LINE: dimension = bg_dimensions;
		...
		{
			WzObject WorldMap = nl.nx.wzFile_map["WorldMap"][map + ".img"];

			if (WorldMap == null)
			{
				WorldMap = nl.nx.wzFile_map["WorldMap"]["WorldMap.img"];
			}
			...
			WzObject mapImage = nl.nx.wzFile_map["MapHelper.img"]["worldMap"]["mapImage"];
			...
			foreach (var list in WorldMap["MapList"])
			{
				...
				if (desc == null && title == null)
				{
					...
					map_spots.Add (new Tuple<Point<short>, MapSpot> (spot, new MapSpot (map_info.description, path, map_info.full_name, type, marker, true, map_ids)));
				}
				else
				{
					map_spots.Add (new Tuple<Point<short>, MapSpot> (spot, new MapSpot (desc.ToString (), path, title.ToString (), type, marker, false, map_ids)));
				}
			}
		}
		...
		private List<System.Tuple<Point<short>, MapSpot>> map_spots = new List<System.Tuple<Point<short>, MapSpot>> ();

		private Point<short> bg_dimensions = new Point<short> ();
		private Point<short> bg_search_dimensions = new Point<short> ();
		private Point<short> background_dimensions = new Point<short> ();
		private Point<short> base_position = new Point<short> ();
	}
}
