...
namespace ms
{
	public class MapTooltip : Tooltip
	{
		public MapTooltip()
		{
			...
			this.fillheight = 0;
			WzObject Frame = nl.nx.wzFile_ui["UIToolTip.img"]["Item"]["Frame2"];
			WzObject WorldMap = nl.nx.wzFile_ui["UIWindow2.img"]["ToolTip"]["WorldMap"];
...
//ORIGINAL LINE: void draw(Point<short> pos) const override
		public override void draw(Point<short> pos)
		{
			...
			if (parent == Tooltip.Parent.MINIMAP && mob_labels.Length == 0 && npc_labels.Length == 0)
			{
				if (desc_label.empty())
				{
					...
					frame.draw(pos + new Point<short>((short)(new_width / 2 + 2), (short)(new_height - 7)), (short)(new_width - 14), (short)(new_height - 18));
					...
					{
						name_simple.draw(pos + new Point<short>(1, -3));
					}
				}
				else
				{
					...
					frame.draw(pos + new Point<short>((short)(half_width + 2), (short)(new_height - 7 + BOTTOM_PADDING)), (short)(new_width - 14), (short)(new_height - 18 + BOTTOM_PADDING));
					cover.draw(pos + new Point<short>(-5, -2));
					name_label.draw(pos + new Point<short>(half_width, -2));

					pos.shift_y(name_height);

					separator.draw(pos + SEPARATOR_ADJ);
					desc_simple.draw(pos + new Point<short>(half_width, -3));
				}
			}
			else
			{
				...
				frame.draw(pos + new Point<short>((short)(half_width + 2), (short)(fillheight - 7 + BOTTOM_PADDING)), (short)(fillwidth - 14), (short)(fillheight - 18 + BOTTOM_PADDING));
				cover.draw(pos + new Point<short>(-5, -2));
				name_label.draw(pos + new Point<short>(half_width, 0));
				...
				if (!desc_label.empty())
				{
					...
					desc_label.draw(pos + new Point<short>(4, 0));
		...
		private static  Point<short> SEPARATOR_ADJ = new Point<short>(5, 0);
		private static  Point<short> LIFE_LABEL_ADJ = new Point<short>(20, 3);
		private static  Point<short> LIFE_ICON_ADJ = new Point<short>(5, 9);
