...
//ORIGINAL LINE: void draw(Point<short> pos) const override
		public override void draw (Point<short> pos)
		{
			...
			frame.draw (pos + new Point<short> (176, (short)(height + 11)), width, (short)(height - 1));
			name.draw (pos + new Point<short> (33, 3));
			cover.draw (pos + new Point<short> (16, -1));

			pos.shift_y (icon_offset);

			box.draw (new DrawArgument (pos + new Point<short> (26, 21)));
			icon.draw (new DrawArgument (pos + new Point<short> (28, 87), 2.0f, 2.0f));
			desc.draw (pos + new Point<short> (102, 15));

			pos.shift_y (level_offset);

			line.draw (pos + new Point<short> (22, 10));
			leveldesc.draw (pos + new Point<short> (25, 11));
		}
		...
		public void set_skill (int id, int level, int mlevel, long expiration)
		{
			...
			if (masterlevel > 0)
			{
				...
				int mlpos = descstr.IndexOf (':', mlstart) + 2;
