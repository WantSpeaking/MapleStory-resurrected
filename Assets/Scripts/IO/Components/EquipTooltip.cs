...
namespace ms
{
	public class EquipTooltip : Tooltip
	{
		public EquipTooltip ()
		{
			...
			reqstatpositions[MapleStat.Id.LEVEL] = new Point<short> (97, 47);
			reqstatpositions[MapleStat.Id.STR] = new Point<short> (97, 62);
			reqstatpositions[MapleStat.Id.LUK] = new Point<short> (177, 62);
			reqstatpositions[MapleStat.Id.DEX] = new Point<short> (97, 71);
			reqstatpositions[MapleStat.Id.INT] = new Point<short> (177, 71);


//ORIGINAL LINE: reqset[false] = Charset(EquipCannot, Charset::Alignment::LEFT);
			reqset[false] = new Charset (EquipCannot, Charset.Alignment.LEFT);

//ORIGINAL LINE: reqset[true] = Charset(EquipCan, Charset::Alignment::LEFT);
			reqset[true] = new Charset (EquipCan, Charset.Alignment.LEFT);


//ORIGINAL LINE: lvset[false] = Charset(EquipCannot, Charset::Alignment::LEFT);
			lvset[false] = new Charset (EquipCannot, Charset.Alignment.LEFT);

//ORIGINAL LINE: lvset[true] = Charset(Equip["YellowNumber"], Charset::Alignment::LEFT);
			lvset[true] = new Charset (Equip["YellowNumber"], Charset.Alignment.LEFT);


//ORIGINAL LINE: atkincset[false] = Charset(Equip["Summary"]["decline"], Charset::Alignment::RIGHT);
			atkincset[false] = new Charset (Equip["Summary"]["decline"], Charset.Alignment.RIGHT);

//ORIGINAL LINE: atkincset[true] = Charset(Equip["Summary"]["incline"], Charset::Alignment::RIGHT);
...
//ORIGINAL LINE: draw_preview(pos);
			draw_preview (pos);
			...
			top.draw (pos);
			mid.draw (new DrawArgument (pos + new Point<short> (0, 13), new Point<short> (0, height)));
			bot.draw (pos + new Point<short> (0, (short)(height + 13)));
			cover.draw (pos);

			name.draw (pos + new Point<short> (17, 7));

			if (prank != Equip.Potential.POT_NONE)
			{
				potflag.draw (pos + new Point<short> (130, 20));
				pos.shift_y (16);
			}
			...
			atkinc.draw (pos + new Point<short> (248, 4));
			baseTexture.draw (pos + new Point<short> (12, 10));
			type[false].draw (pos + new Point<short> (12, 10));
			itemicon.draw (new DrawArgument (pos + new Point<short> (18, 82), 2.0f, 2.0f));
			potential[prank].draw (pos + new Point<short> (12, 10));
			itemcover.draw (pos + new Point<short> (12, 10));
			...
			atkincset[atkinc_pos].draw (atkincstr, 11, pos + new Point<short> (239, 26));

			pos.shift_y (12);

			foreach (MapleStat.Id ms in requirements)
			{
				Point<short> reqpos = reqstatpositions[ms];
				bool reqok = canequip[ms];
				reqstattextures[ms][reqok].draw (pos + reqpos);

				if (ms != MapleStat.Id.LEVEL)
				{
					reqset[reqok].draw (reqstatstrings[ms], 6, pos + reqpos + new Point<short> (54, 0));
				}
				else
				{
					lvset[reqok].draw (reqstatstrings[ms], 6, pos + reqpos + new Point<short> (54, 0));
				}
			}
			...
			line.draw (pos + new Point<short> (0, 47));
			...
			category.draw (pos + new Point<short> (stat_x, 0));

			pos.shift_y (stat_y);

			if (is_weapon)
			{
				wepspeed.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
			}
			...
			foreach (Text label in statlabels.dict.Values)
			{
				...
				label.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
			}

			if (hasslots)
			{
				slots.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
				hammers.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
			}
			...
			if (hasdesc)
			{
				pos.shift_y (13);
				line.draw (pos);
				desc.draw (pos + new Point<short> (9, 3));
			}
		}

//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: void draw_preview(Point<short> pos) const
		public void draw_preview (Point<short> pos)
		{
			if (invpos_preview == 0)
			{
				return;
			...
			top.draw (pos);
			mid.draw (new DrawArgument (pos + new Point<short> (0, 13), new Point<short> (0, height_preview)));
			bot.draw (pos + new Point<short> (0, (short)(height_preview + 13)));
			cover.draw (pos);

			name_preview.draw (pos + new Point<short> (17, 7));

			if (prank_preview != Equip.Potential.POT_NONE)
			{
				potflag_preview.draw (pos + new Point<short> (130, 20));
				pos.shift_y (16);
			}
			...
			atkinc_preview.draw (pos + new Point<short> (248, 4));
			baseTexture.draw (pos + new Point<short> (12, 10));
			type[false].draw (pos + new Point<short> (12, 10));
			itemicon_preview.draw (new DrawArgument (pos + new Point<short> (18, 82), 2.0f, 2.0f));
			potential[prank_preview].draw (pos + new Point<short> (12, 10));
			itemcover.draw (pos + new Point<short> (12, 10));
			...
			foreach (MapleStat.Id ms in requirements)
			{
				...
				if (ms != MapleStat.Id.LEVEL)
				{
					reqset[reqok].draw (reqstatstrings_preview[ms], 6, pos + reqpos + new Point<short> (54, 0));
				}
				else
				{
					lvset[reqok].draw (reqstatstrings_preview[ms], 6, pos + reqpos + new Point<short> (54, 0));
				}
			}
			...
			category_preview.draw (pos + new Point<short> (stat_x, 0));

			pos.shift_y (stat_y);

			if (is_weapon_preview)
			{
				wepspeed_preview.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
			}
			...
			if (hasslots_preview)
			{
				slots_preview.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
				hammers_preview.draw (pos + new Point<short> (stat_x, 0));
				pos.shift_y (stat_y);
			}
			...
			if (hasdesc_preview)
			{
				pos.shift_y (-4);
				line.draw (pos);
				desc_preview.draw (pos + new Point<short> (9, 8));
			}
		}
		...
		private BoolPair<Charset> reqset = new BoolPair<Charset> ();
		private BoolPair<Charset> lvset = new BoolPair<Charset> ();
