...
namespace ms
{
	...
	{
		public MovementPacket(OutPacket.Opcode opc) : base((short)opc)
		{
		}

		protected void writemovement(Movement movement)
		{
			write_byte((sbyte)movement.command);
			write_short(movement.xpos);
			write_short(movement.ypos);
			write_short(movement.lastx);
			write_short(movement.lasty);
			write_short((short)movement.fh);
			write_byte((sbyte)movement.newstate);
