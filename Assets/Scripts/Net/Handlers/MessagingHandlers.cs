...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: 'const' methods are not available in C#:
//ORIGINAL LINE: void show_status(Color::Name color, const string& message) const
		private void show_status (Color.Name color, string message)
		{
			/*todo var messenger = UI.get ().get_element<UIStatusMessenger> ();
			if (messenger)
			{
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle (InPacket recv)
		{
			...
			//todo UI.get ().get_element<UIChatBar> ().Dereference ().send_chatline (message, UIChatBar.LineType.YELLOW);
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle (InPacket recv)
		{
			...
			/*todo var character = Stage.get ().get_character (charid);
			if (character)
			{
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle (InPacket recv)
		{
			...
			else // Buff effect
			{
				...
				//todo Stage.get ().get_combat ().show_player_buff (skillid);
			}
		}
	}
}