...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_chars().spawn({cid, look, level, job, name, stance, position});
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_chars().remove(cid);
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			int cid = recv.read_int();
			recv.skip(4);
			//todo List<Movement> movements = MovementParser.parse_movements(recv);

			//todo Stage.get().get_chars().send_movement(cid, movements);
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_chars().update_look(cid, look);
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			{
				//todo Stage.get().show_character_effect(cid, CharEffect.Id.MONSTER_CARD);
			}
			else if (recv.available()) // skill
			{
				...
				//todo Stage.get().get_combat().show_buff(cid, skillid, effect);
			}
			else
			{
				// TODO: Blank
...
//ORIGINAL LINE: Stage::get().get_mobs().send_movement(oid, position, move(movements));
			//todo Stage.get().get_mobs().send_movement(oid, position, move(movements));
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_npcs().spawn({oid, id, position, flip, fh});
		}
	}
...
//ORIGINAL LINE: dropfrom = recv.read_point();
...
//ORIGINAL LINE: dropfrom = dropto;
			...
			//todo Stage.get().get_drops().spawn({oid, itemid, meso, owner, dropfrom, dropto, pickuptype, mode, playerdrop});
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			{
				int cid = recv.read_int();
				//todo var character = Stage.get ().get_character (cid);
				if (recv.length() > 0)
				{
					recv.read_byte(); // pet
				}
			...
			//todo Stage.get().get_drops().remove(oid, mode, looter.get());
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_reactors().trigger(oid, state);
		}
	}
...
//ORIGINAL LINE: void handle(InPacket& recv) const override
		public override void handle(InPacket recv)
		{
			...
			//todo Stage.get().get_reactors().spawn({oid, rid, state, point});
		}
	}
...
//C++ TO C# CONVERTER CRACKED BY X-CRACKER 2017 WARNING: The following line was determined to be a copy constructor call - this should be verified and a copy constructor should be created if it does not yet exist:
//ORIGINAL LINE: Stage::get().get_reactors().remove(oid, state, point);
			//todo Stage.get().get_reactors().remove(oid, state, point);
		}
	}
}
