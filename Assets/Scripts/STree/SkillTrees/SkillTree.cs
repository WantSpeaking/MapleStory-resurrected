...
namespace ms.SkillTrees
{
    ...
    [CreateAssetMenu(menuName = "ParadoxNotion/NodeCanvas/ST Asset")]
    public class SkillTree : Graph
    {
        ...
        private Stack<STState> stateStack;
        ...
        public STState currentState { get; set; }
        ///<summary>The previous FSM state</summary>
        public STState previousState { get; set; }
        ...
        List<Node> eligibleSTStates = new List<Node>();
        List<Node> tempSTStates = new List<Node>();
        protected override void OnGraphStarted() {
            stateStack = new Stack<STState>();
        ...
        {
            tempSTStates.Clear();
            tempSTStates.AddRange(allNodes);
            allNodes.ForEach(n => n.SetStatus(Status.Failure));
            FindEligibleStates();
        }
        protected override void OnGraphUpdate() {
            ...
            /*if ( enterStartStateFlag ) {
                ...
                EnterState((STState)primeNode, TransitionCallMode.Normal);
            }
        ...
        public bool EnterState(STState newState, TransitionCallMode callMode) {
        ...
        public STState TriggerState(string stateName, TransitionCallMode callMode) {
        ...
        public string[] GetStateNames() {
            return allNodes.Where(n => n is STState).Select(n => n.name).ToArray();
        }

        ///<summary>Get a state by it's name</summary>
        public STState GetStateWithName(string name) {
            return (STState)allNodes.Find(n => n is STState && n.name == name);
        }
        ...
        public STState PeekStack() {
            return stateStack != null && stateStack.Count > 0 ? stateStack.Peek() : null;
        }

        public void FindEligibleStates()
        {
            foreach ( var state in tempSTStates)
            {
                STState sTState = state as STState;
                if ( sTState.ISEligible() ) { eligibleSTStates.Add(sTState); }
            }


            if (eligibleSTStates.Any()) 
            {
                eligibleSTStates.ForEach(i => { tempSTStates.Remove(i); i.SetStatus(Status.Success); Debug.Log(i.name); });

                eligibleSTStates.Clear();
                FindEligibleStates();
            }
        ...
        [UnityEditor.MenuItem("Tools/ParadoxNotion/NodeCanvas/Create/SkillTree Asset", false, 1)]
        static void Editor_CreateGraph() {
            var newGraph = EditorUtils.CreateAsset<SkillTree>();
